      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {testimonials.map((testimonial) => (
            <Card
              key={testimonial.id}
              className="border-white/10 bg-card/90 hover:border-primary/50 transition-all duration-300"
            >
              <CardContent className="p-4 space-y-3">
                <Quote className="h-5 w-5 text-primary opacity-20" />
                <p className="text-gray-300 italic text-xs">"{testimonial.content}"</p>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/10">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-primary/50 flex items-center justify-center text-xs font-semibold text-white">
                    {testimonial.avatar}
                  </div>
                  <div>
                    <p className="text-white font-semibold text-xs">{testimonial.name}</p>
                    <p className="text-gray-400 text-[10px]">{testimonial.role}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
